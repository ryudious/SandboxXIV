...
// Type: SandboxXIV.Editors.MemoryEditor
...
namespace SandboxXIV.Editors
{
  public class MemoryEditor : Editor
  {
    ...
    public MemoryEditor()
    {
      ...
      if (SandboxXIV.SandboxXIV.Config.HelpMeIveFallenAndICantGetUp)
      {
        foreach (Memory.ReplacerBuilder customReplacer in SandboxXIV.SandboxXIV.Config.CustomReplacers)
          customReplacer.AutoEnable = false;
      }
      SandboxXIV.SandboxXIV.Config.HelpMeIveFallenAndICantGetUp = true;
      SandboxXIV.SandboxXIV.Config.Save();
      foreach (Memory.ReplacerBuilder customReplacer in SandboxXIV.SandboxXIV.Config.CustomReplacers)
        this.editorReplacers.Add((customReplacer, customReplacer.ToReplacer()));
      SandboxXIV.SandboxXIV.Config.HelpMeIveFallenAndICantGetUp = false;
      SandboxXIV.SandboxXIV.Config.Save();
    }
    ...
    protected override void Draw()
    {
      ...
      if (ImGui.BeginTabItem("View"))
      {
        ...
        double x = (double) ((ImGuiStylePtr) ref style).ItemSpacing.X;
        ...
        if (this.editorAddress != IntPtr.Zero)
        {
          for (int index1 = -2; index1 < 4; ++index1)
          {
            int num3 = 32 * index1;
            byte[] numArray;
            if (SafeMemory.ReadBytes(this.editorAddress + num3, 32, ref numArray))
            {
      ...
      if (ImGui.BeginTabItem("Replacers"))
      {
        ...
        void AddReplacerEditor(int id, Memory.ReplacerBuilder builder, Memory.Replacer replacer)
        {
          ...
          if (flag1)
          {
            if (ImGui.Button("Add"))
            {
              SandboxXIV.SandboxXIV.Config.CustomReplacers.Add(builder);
          ...
          else if (ImGui.Button("Delete"))
          {
            ...
            SandboxXIV.SandboxXIV.Config.CustomReplacers.Remove(builder);
            SandboxXIV.SandboxXIV.Config.Save();
          }
          ImGui.PopID();
        }
